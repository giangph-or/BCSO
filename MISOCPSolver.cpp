...
void MISOCPSolver::solve(string output, int time_limit) {
    try {
        ...
        report_results << data.T << "," << data.m << "," << data.W << "," << param.M << "," << param.K
            << "," << model.get(GRB_IntAttr_Status) << setprecision(8) << fixed << "," << obj_val_gurobi
            << ","
            << -obj_val_true << "," << time_for_solve << "," << /*time_for_param*/ 0 << ","
            << /*time_for_mc*/ 0 << ","
            << gap << endl;
