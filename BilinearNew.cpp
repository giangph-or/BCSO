...
BilinearNew::BilinearNew() {

}

BilinearNew::BilinearNew(Data data, int K, double tol_lamda, int M) {
    this->data = data;
...
void BilinearNew::solve(string output, int time_limit) {
    try {
        ...
        // f=n/d
        for (int iter_t = 0; iter_t < data.T; iter_t++)
            model.addQConstr(n[iter_t] == f[iter_t] * d[iter_t]);
